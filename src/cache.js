...
const handleCache = async ({ ttl, doc, key, cache, allCacheKeys, debug, allowFlushingCollectionCache }) => {
...
export const setupCaching = ({ collection, cache, allowFlushingCollectionCache = false, mongoose = false, debug = false }) => {
  ...
  const cachePrefix = `mongo-${collection.collectionName ? collection.collectionName : collection.modelName ? collection.modelName : 'test'}-`
  ...
  collection.findOneById = async (id, { ttl } = {}) => {
    ...
    await handleCache({ ttl, doc, key, cache, allCacheKeys, debug, allowFlushingCollectionCache })
  ...
  const dataQuery = mongoose
    ? ({ queries }) => collection.find({ $or: queries })
  ...
  collection.findManyByQuery = async (query, { ttl } = {}) => {
    ...
    await handleCache({ ttl, doc: docs, key, cache, allCacheKeys, debug, allowFlushingCollectionCache })
    return docs
  }


  collection.deleteFromCacheById = async (id) => {
    const key = id && typeof id === 'object'
      ? JSON.stringify(id)
      : id;
    const allKeys = (await cache.get(allCacheKeys)) || [];
